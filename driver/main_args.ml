...
let mk_pure_caml f =
  "-pure_caml", Arg.Unit f, " (undocumented)"
;;

let mk_lexifi_extensions f =
  "-lexifi-extensions", Arg.Unit f, " enable (some) LexiFi extensions" ^ (if Config.pure_caml then "" else " (default)")
;;

let mk_no_lexifi_extensions f =
  "-no-lexifi-extensions", Arg.Unit f, " disable (some) LexiFi extensions" ^ (if Config.pure_caml then " (default)" else "")
;;
...
let mk_drop_modules f =
  "-drop-modules", Arg.String f,
  "<modules> when creating a library, ignore modules listed in the argument (newline-separated) [LEXIFI EXTENSION]"
;;
...
struct
  let list = [
    ...
    mk_pure_caml F._pure_caml;
    mk_lexifi_extensions F._lexifi_extensions;
    mk_no_lexifi_extensions F._no_lexifi_extensions;
...
struct
  let list = [
    ...
    mk_drop_modules F._drop_modules;
...
module Default = struct
  ...
  module Core = struct
    ...
    let _pure_caml = set pure_caml
    let _lexifi_extensions = clear pure_caml
    let _no_lexifi_extensions = set pure_caml
  ...
  module Compiler = struct
    ...
    let _drop_modules s = drop_modules := Some s
