...
CAMLprim value caml_float_of_string(value vs)
{
  ...
  /* Fast path for numbers of length <= N+1 with only decimal digits
     (N = 9 in 32-bit, 16 in 64-bit) and exactly one dot,
     possibly prefixed by '+' or '-' and a sequence of '0' before the
     dot.

     N is chosen so that:

     (1) the largest integer with N digits (99...99) can be
         represented as a native integer (which forces N <= 9 on
         32-bit, N <= 18 on 64-bit);

     (2) all integers with at most N digits can be represented exactly
         as a double (the upperbound is 2^53, which forces N <= 15).

     Possible improvements:

     - Allow one more digit in 64-bit, since most integers with 16
       digits are below 2^53, and check at the end that the integer is
       indeed <= 2^53 (or even that it can roundtrip to and from
       double).

     - Drop trailing zeroes.

     - Drop leading zeroes after the dot, so that
       e.g. 0.031400000000000004 (obtained as 3.14/100) would be considered
       as having 17 digits.
  ...
  */
  ...
  if (*src == '-') { sign = -1; src++; }
  while (*src == '0') src++;
  if (caml_string_length(vs) - (src - String_val(vs)) <= (sizeof(intnat)==4 ? 10 : 16)) {
    intnat res = 0;
    const char *dot;
    char c;
    ...
    if (!res)
      /* Special case, required to parse -0. correctly */
      d = (sign < 0 ? -0. : 0.);
    else
      /* src - dot - 1 is the number of digits parsed after the dot */
      d = ((double) (sign < 0 ? -res : res)) / pow10_[src - dot - 1];

    /*
    if (d != strtod((const char *) String_val(vs), NULL)) {
      printf("*** KO '%s' ***\n", String_val(vs));
      fflush(stdout);
      exit(2);
    }
