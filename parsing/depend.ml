...
let rec add_type bv ty =
  ...
  (* BEGIN LEXIFI *)
  | Ptyp_constr({txt = Lident ("ttype" as s); loc}, [t]) ->
      add bv {txt = Ldot (Location.mknoloc (Lident "Mlfi_types"), Location.mknoloc s); loc};
      add_type bv t
  (* END LEXIFI *)
...
let rec add_expr bv exp =
  ...
  | Pexp_extension ({txt="p"},PStr str) ->
      add_module_path bv (mknoloc (Lident "Mlfi_type_path"));
      add_implementation bv str
  | Pexp_extension ({txt="lazy"|"t"},PStr str) ->
      add_implementation bv str
  | Pexp_extension ({txt="fields_of"|"t"}, PTyp sty) ->
      add_module_path bv (mknoloc (Lident "Mlfi_types"));
      add_type bv sty
...
and add_struct_item (bv, m) item : _ String.Map.t * _ String.Map.t =
  ...
  | Pstr_include incl ->
      ...
      let add = String.Map.fold String.Map.add m' in
      (add bv, add m)
  | Pstr_extension (({txt="t"}, PStr[{pstr_desc=Pstr_value(_,[{pvb_pat=p;pvb_expr=e}])}]),_) ->
