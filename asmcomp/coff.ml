...
[@@@ocaml.warning "+A-42-4-9-27-32-60-69"]
...
let blit_int32 s pos i =
  Bytes.set s pos (Char.chr (Int32.to_int (i &&& 0xffl)));
  Bytes.set s (pos+1) (Char.chr (Int32.to_int ((i >>> 8) &&& 0xffl)));
  Bytes.set s (pos+2) (Char.chr (Int32.to_int ((i >>> 16) &&& 0xffl)));
  Bytes.set s (pos+3) (Char.chr (Int32.to_int ((i >>> 24) &&& 0xffl)))
...
(*
(* Contrary to flexdll, we emit to a local buffer instead of an out_channel *)
...
let output_buffer oc b = Buffer.add_string oc (Buffer.contents b)
...
let is_zero s =
  try
    for i = 0 to Bytes.length s - 1 do
      if Bytes.get s i <> '\000' then raise Exit
    done;
    true
  with Exit -> false
...
let read ic pos len =
  if len = 0 then ""
  else (seek_in ic pos;  really_input_string ic len)
...
let extract_buf oc =
  let s = Buffer.to_bytes oc in
  List.iter (fun (pos, i) -> blit_int32 s pos i) !patches;
  patches := [];
  Bytes.to_string s
...
module Symbol = struct
  ...
  let get (strtbl : int -> string) ic pos =
...
module Section = struct
  let set_text sect s = sect.data <- `String s
...
module Coff = struct
  ...
  let put oc x =
    ...
    output_buffer oc strbuf;

    List.iter (fun s -> s.sym_pos <- 0) x.symbols;
    List.iter (fun s -> s.sec_pos <- 0) x.sections

  let filter_symbols coff f =
    coff.symbols <- List.filter (fun sym -> f sym.sym_name) coff.symbols
end
