...
let create_archive file_list lib_name =
  (* BEGIN LEXIFI *)
  let file_list =
    match !Clflags.drop_modules with
    | None -> file_list
    | Some files ->
        let tbl = Hashtbl.create 8 in
        List.iter (fun s -> if s <> "" then Hashtbl.replace tbl (s ^ ".cmx") ()) (String.split_on_char '\n' files);
        let l = List.filter (fun f -> let f = Filename.basename f in if Hashtbl.mem tbl f then (Hashtbl.remove tbl f; false) else true) file_list in
        if Hashtbl.length tbl > 0 then begin
          Hashtbl.iter (fun f _ -> Printf.eprintf "ERROR: cannot drop %s\n%!" f) tbl;
          exit 2;
        end;
        l
  in
