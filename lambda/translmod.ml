...
let record_primitive = function
  | {val_kind=Val_prim ({Primitive.prim_name = "%typeof"; _} as p); _} ->
...
let transl_implementation_flambda module_name (str, cc) =
  ...
  (* BEGIN LEXIFI *)
  let body = Transldynamic.add_mlfi_specific_to_module (Transldynamic.lift_ttype_exprs body) in
  (* END LEXIFI *)
...
(* BEGIN LEXIFI *)
let lambda_subst subst lam = Lambda.subst (fun _ _ env -> env) subst (Transldynamic.lift_ttype_exprs lam)
(* END LEXIFI *)
                  ...
                  | id :: idl ->
                      Llet(Alias, Pgenval, id, Lvar ids.(pos),
                           Lsequence(store_ident loc id,
                                     store_idents (pos + 1) idl))
                in
                Lsequence(lam, lambda_subst subst
                                 (store_idents 0 ids0))
            | _ ->
                let pure = pure_module od.open_expr in
                (* this optimization shouldn't be needed because Simplif would
...
let transl_store_gen ~scopes module_name ({ str_items = str }, restr) topl =
  ...
  let f = function
    | [ { str_desc = Tstr_eval (expr, _attrs) } ] when topl ->
        assert (size = 0);
        lambda_subst !transl_store_subst
          (transl_exp ~scopes expr)
    | str -> transl_store_structure ~scopes module_id map prims aliases str
  in
  (* BEGIN LEXIFI *)
  let f str =
    Transldynamic.add_mlfi_specific_to_module (f str)
  in
  (* END LEXIFI *)
...
let transl_toplevel_item_and_close ~scopes itm =
  close_toplevel_term
    (Transldynamic.add_mlfi_specific_to_module
