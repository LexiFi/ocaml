...
let rec print_out_type ppf =
  ...
  | Otyp_poly (sl, ty) ->
      fprintf ppf "@[<hov 2>%a.@ %a@]"
        ...
        print_out_type ty
  | ty ->
      print_out_type_1_ ppf ty

and print_out_type_1_ ppf ty =
  match ty with
  | Otyp_arrow _ -> fprintf ppf "@[<hv>%a@]" print_out_type_1 ty
  | _ -> print_out_type_1 ppf ty
...
and print_out_sig_item ppf =
  ...
  | Osig_value vd ->
      ...
      in
      let pr_val ppf = function
        | None -> ()
        | Some s -> fprintf ppf "@ = %S" s
      in
      fprintf ppf "@[<2>%s %a :@ %a%a%a%a@]" kwd value_ident vd.oval_name
        !out_type
        vd.oval_type pr_prims prims pr_val v
