...
let rec find_repr p1 =
  function
    Mnil ->
      None
  | Mcons (Public, p2, ty, _, rp, _)
    when Path.same_unsafe p1 p2 && rp = !remove_props ->
      Some ty
  | Mcons (_, _, _, _, _, rem) ->
      find_repr p1 rem
  | Mlink {contents = rem} ->
...
let expand_abbrev_gen kind find_type_expansion env ty =
  ...
  | None ->
      ...
      | (params, body, expansion_scope, attrs) ->
          (* prerr_endline
...
let rec has_cached_expansion p abbrev =
  match abbrev with
    Mnil                    -> false
  | Mcons(_, p', _, _, rp, rem) ->
      (rp = !remove_props && Path.same_unsafe p p')
