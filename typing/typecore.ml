...
let with_level_for_implicit_ttype f =
  Misc.protect_refs [R (level_for_implicit_ttype, Some (Ctype.get_current_level ()))] f
...
let remaining_function_type_for_error ty_ret rev_args =
  List.fold_left
    (fun ty_ret (lbl, arg) ->
        match arg with
        | Arg (Unknown_arg _ | Typed_arg _ | Known_arg _ | Implicit_arg _) ->
            ty_ret
...
let collect_arrow_arg ~may_warn ~funct ~optional ~sargs ~ty_arg ~ty_arg0 ~lv
  ...
  | None ->
      ...
      end else if Typedynamic.has_implicit funct.exp_env ty_arg
                && has_non_labelled then begin
        Arg (Implicit_arg { ty_arg = ty_arg0; env = funct.exp_env;
                            loc = funct.exp_loc })
      end else begin
...
and type_expect_
  ...
  | Pexp_struct_item ({pstr_desc; _} as si, e) ->
      let tv = newvar () in
      let (_, si, exp) =
        let with_level_for_implicit_ttype f =
          match pstr_desc with
          | Pstr_module _ | Pstr_open { popen_expr = { pmod_desc = Pmod_structure _; _ }; _ } ->
              with_level_for_implicit_ttype f
          | _ -> f ()
        in
        with_local_level_generalize begin fun () ->
          let (si, newenv) =
          ...
          let exp =
            with_level_for_implicit_ttype
              (fun () -> type_expect newenv e ty_expected_explained)
          in
          (newenv, si, exp)
...
and type_implicit_arg env loc ty =
  ...
  | Auto_ttype t ->
      ...
      e
  | Auto_call_site ->
      (* Format.eprintf "expected = %a@." Printtyp.type_expr ty; *)
      type_expect env (call_site ()) (mk_expected (Ctype.maybe_instance_poly ty))
  | Auto_none ->
      assert false
