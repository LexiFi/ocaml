...
type record_repr = Record_regular | Record_float | Record_inline of int | Record_unboxed
...
let is_enumeration = function
  ...
  | DT_node{rec_descr = DT_variant {variant_constrs; variant_repr=_}; _} -> List.for_all (fun (_, _, args) -> args = C_tuple []) variant_constrs
  | _ -> false
...
module Internal = struct
  ...
  let dummy_descr = DT_variant {variant_constrs=[]; variant_repr = Variant_regular}
  let uid = ref 0
  ...
  let set_node_variant n (constrs, repr) =
    n.rec_descr <- DT_variant {variant_constrs=constrs; variant_repr=repr}
  ...
  let map_node ~ignore_props aux memo n =
    ...
    n'.rec_descr <- begin
      ...
      | DT_variant v ->
          DT_variant {v with variant_constrs = List.map (fun (s, p, args) ->
...
module Textual = struct
  ...
  type node =
    | Variant of string * t list * (string * stype_properties * t variant_args) list * variant_repr
